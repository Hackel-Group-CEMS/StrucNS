...
OUTPUT_DIRECTORY = "graphml_mutants_random_walk" # Updated output dir name for clarity
NUM_CHUNKS = 100 
GLOBAL_CHECKPOINT_FILE = "processed_pdbs.txt" 
# ----------------------------------------
...
# --- NEW: Random Walk Function ---
def apply_random_walk_geometry(G):
    """
    Overwrites the 'pos' (centroid) of every node in G with a 
    Self-Avoiding Random Walk (Chain Growth).
    
    Strategy:
    1. Start Residue 1 at (0,0,0).
    2. Place Residue i at a random direction 2.0-4.0 A away from Residue i-1.
    3. Preserves peptide bond continuity but destroys 3D folding.
    """
    nodes = list(G.nodes()) # PDB parsing usually preserves order
    num_nodes = len(nodes)
    ...
    # Update first node
    G.nodes[nodes[0]]['pos'] = tuple(current_pos)
    # Also update split coordinates for consistency
    G.nodes[nodes[0]]['x'] = current_pos[0]
    G.nodes[nodes[0]]['y'] = current_pos[1]
    G.nodes[nodes[0]]['z'] = current_pos[2]

    # 2. Walk the Chain
    for i in range(1, num_nodes):
        # Generate random direction (uniform on sphere)
        # Using Gaussian sampling normalized
        direction = np.random.normal(0, 1, 3)
        norm = np.linalg.norm(direction)
        if norm < 1e-6: # Edge case
             direction = np.array([1.0, 0.0, 0.0])
             norm = 1.0
        unit_vec = direction / norm
        
        # Random step size between 2.0 and 4.0 Angstroms (Physically valid peptide bond range)
        step_size = np.random.uniform(2.0, 4.0)
        
        # Update Position
        current_pos = current_pos + (unit_vec * step_size)
        
        # Update Node Attributes
        G.nodes[nodes[i]]['pos'] = tuple(current_pos)
        G.nodes[nodes[i]]['x'] = current_pos[0]
        G.nodes[nodes[i]]['y'] = current_pos[1]
        G.nodes[nodes[i]]['z'] = current_pos[2]
        
    return G

# --- Edge Helper Functions (Unchanged) ---
...
def get_ring_centroids(ring_atoms_df: pd.DataFrame) -> pd.DataFrame:
    return ring_atoms_df.groupby("node_id").agg({
        "x": "mean", "y": "mean", "z": "mean", "residue_name": "first"
    }).reset_index()
...
# --- MODIFIED: Network Generation Function ---
def generate_network_from_pdb(pdb_file):
    ...
    for model in structure:
        for chain in model:
            ...
            for residue in chain:
                ...
                # Calculate original centroid for temporary storage
                ...
                # Store atom info for chemical edges (Interactions still use REAL atoms)
                for atom in residue:
                    if atom.element != 'H':
                        rgroup_data.append({
    ...
    # Apply Random Walk Geometry immediately after parsing nodes.
    # This overwrites the 'pos' attribute of the nodes.
    # Result: Nodes are in a random chain, but rgroup_df (atoms) is still original.
    G = apply_random_walk_geometry(G)
    # --- KEY MODIFICATION END ---

    # Add Generic Distance Edges (Based on NEW Random Walk Geometry)
    # Note: Since the random walk is sparse/unfolded, this will find very few edges (mostly neighbors).
    for node1, data1 in G.nodes(data=True):
        for node2, data2 in G.nodes(data=True):
            if node1 != node2:
                # This uses the NEW 'pos' (Random Walk)
                distance = np.linalg.norm(np.array(data1['pos']) - np.array(data2['pos']))
                if distance < 5.0 and not G.has_edge(node1, node2):
                    G.add_edge(node1, node2, kind=set())

    # Add Specific Chemical Interactions (Based on ORIGINAL Atoms)
    # These functions use G.graph["rgroup_df"], which retains the REAL PDB atomic coordinates.
    # Result: The graph retains its chemical wiring (H-bonds, etc.) even though the geometry is destroyed.
    ...
    with open(CHUNK_CHECKPOINT_FILE, 'a') as chunk_checkpoint:
        for pdb_file in pdb_files_in_chunk:
            try:
                ...
                print(f"Processing (Random Walk Geometry): {pdb_file}")
                pdb_path = os.path.join(input_dir, pdb_file)

                # Generate the graph (NOW WITH RANDOM WALK)
